...
async function logVisitorData() {
  const endpoint = "https://script.google.com/macros/s/AKfycbwisxQoPW6uZ6Vhn1QFHhzsCSlQ8UMZQ3L2AAcATfyFgguY-mRUMxthyQLhyP7AHF6d/exec"; 

  const ipData = await fetch("https://api.ipify.org?format=json").then(res => res.json());
  const userAgent = navigator.userAgent;

  let location = "Unknown", connection = "Unknown";

  try {
    const locData = await fetch(`https://ipapi.co/${ipData.ip}/json/`).then(res => res.json());
